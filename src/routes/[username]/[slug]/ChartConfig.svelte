<script lang="ts">
	...
	const handleChartTypeChange = (event: Event) => {
		...
		const type = select.value as 'candle' | 'line' | 'bar' | 'table';
...
<Select bind:this={select} {anchor} placement="bottom-end">
	<form>
		<div class="setting">
			<span>type</span>
			<select multiple value={settings.type} onchange={handleChartTypeChange} size={4}>
		...
		{#if settings.type === 'candle' || settings.type === 'line' || settings.type === 'bar'}
			<div class="setting">
				<span>x-axis</span>
